		...
		fn:function(data){
			...
			function mainLayout(){
				...
				obj.model.get("#main","userCenterTemSimple","userCenterTem",function(model){
					...
					{name:"logistics",title:"快递公司",placeholder:"",type:"select",value:"",valuelabel:"",option:express},
					{name:"logNumber",title:"订单号",placeholder:"",type:"input",value:"",valuelabel:"",option:[]}
					],
					button:[{id:"sendDeal",text:"发货"}]
					});
					model.setResult(deal);	
					model.reflash();
					model.show();
					model.target.find("#sendDeal").unbind("click").bind("click",function(){
						var sendData=model.result();
						sendData.tk=tk;
						obj.api.run("deal_send",sendData,function(){
							obj.pop.on("alert",{text:"提交成功"});
							obj.hash("dealListUc/0");
						},function(e){
