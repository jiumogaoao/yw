		...
		fn:function(data){
			...
			function mainLayout(){
				...
				obj.model.get("#main","userCenterTemSimple","userCenterTem",function(model){
					...
					nav:[],
					list:[
					{name:"number",title:"充值金额",placeholder:"",type:"number",value:"",valuelabel:"",option:[{label:"",value:""}]}
					],
					button:[{id:"rechargeAdd",text:"充值"}]
					});
					model.reflash();
					model.show();
					model.target.find("#rechargeAdd").unbind("click").bind("click",function(){
						...
						if(!sendData.number){
							obj.pop.on("alert",{text:"请先输入充值金额"});
								return false;
						}
						sendData.tk=tk;
						obj.api.run("money_in",sendData,function(returnData){
